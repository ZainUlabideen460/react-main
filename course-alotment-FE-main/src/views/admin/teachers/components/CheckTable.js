 ...
 const [searchField, setSearchField] = useState('none');
  ...
  const exportToExcel = async () => {
    try {
      ...
      const toastId = toast.loading('Preparing teacher export...');
      
      try {
        const response = await axios.get('http://localhost:3001/api/teachers/export', {
          responseType: 'blob',
          headers: {
      ...
      toast.error(`Export failed: ${error.message}`, {
        id: toast.loading ? undefined : null, // Clear loading toast if it exists
        duration: 5000
      });
    }
  };
